...
# Paleta de activos (distintos, fácilmente diferenciables en oscuro)
PALETA_ACTIVOS = [
    "#00D4AA",  # teal
    "#3B82F6",  # blue
    "#F97316",  # orange
...
LAYOUT_BASE = dict(
    ...
    xaxis=dict(
        ...
        tickcolor=COLORES["text_muted"],
        tickfont=dict(size=10, color=COLORES["text_muted"]),
        showgrid=True,
        zeroline=False,
    ),
    yaxis=dict(
    ...
    legend=dict(
        bgcolor="rgba(14,26,46,0.9)",
        bordercolor=COLORES["border"],
        borderwidth=1,
        font=dict(size=10, color=COLORES["text_muted"]),
        orientation="h",
        y=-0.2,
    ),
    hoverlabel=dict(
...
def grafico_distribucion_retornos(resultado: dict) -> go.Figure:
    """Histograma de retornos con zonas VaR/CVaR destacadas."""
    ...
    fig.add_trace(go.Histogram(
        ...
        marker=dict(
            color=COLORES["teal_mid"],
            line=dict(color=COLORES["teal"], width=0.5),
        ),
        opacity=0.85,
        hovertemplate="Retorno: %{x:.3%}<br>Frecuencia: %{y}<extra></extra>",
    ))

    # Zona de pérdida (área roja)
    ...
    if len(x_loss) > 0:
        fig.add_trace(go.Histogram(
            ...
            marker=dict(
                color=COLORES["red_dim"],
                line=dict(color=COLORES["red"], width=0.5),
            ),
            opacity=0.9,
            hovertemplate="Retorno: %{x:.3%}<extra>Zona de pérdida</extra>",
        ))
    ...
    fig.add_vline(
        ...
        line_dash="dash",
        annotation=dict(
            text=f"VaR {confianza:.0%}<br><b>{var_pct:.3%}</b>",
            font=dict(size=10, color=COLORES["orange"]),
            bgcolor=COLORES["bg_card"],
    ...
    fig.add_vline(
        x=cvar_pct,
        line_color=COLORES["red"],
        line_width=1.5,
        line_dash="dot",
        annotation=dict(
            text=f"CVaR<br><b>{cvar_pct:.3%}</b>",
...
def grafico_precios_historicos(resultado: dict) -> go.Figure:
    """Precios históricos normalizados a base 100."""
    ...
    fig.add_hline(
        y=100,
        line_dash="dash",
        line_color=COLORES["text_muted"],
        line_width=1,
        opacity=0.5,
        annotation=dict(
            text="base 100",
            font=dict(size=9, color=COLORES["text_muted"]),
            xanchor="right",
        ),
...
def grafico_retornos_tiempo(resultado: dict) -> go.Figure:
    """Retornos diarios del portafolio con zona de pérdida coloreada."""
    retornos  = resultado["retornos_portafolio"]
    fechas    = resultado["fechas"]
    var_pct   = resultado["var_pct"]
    ...
    # Banda VaR
    fig.add_hrect(
        y0=min(retornos) * 1.05,
        y1=var_pct,
        fillcolor=COLORES["red_dim"],
        line_width=0,
        annotation=dict(
            text="Zona VaR",
            font=dict(size=9, color=COLORES["red"]),
            xanchor="left",
        ),
        annotation_position="top left",
    )
...
def grafico_composicion(pesos: dict) -> go.Figure:
    """Donut chart de composición del portafolio."""
    ...
    fig = go.Figure(go.Pie(
        ...
        marker=dict(
            ...
            line=dict(color=COLORES["bg_surface"], width=2),
        ),
        textinfo="label+percent",
        textfont=dict(size=11, family="'IBM Plex Mono', monospace", color=COLORES["text_primary"]),
        hovertemplate="<b>%{label}</b><br>Peso: %{value:.1%}<br>%{percent}<extra></extra>",
        direction="clockwise",
        sort=True,
    ))
    ...
    # Texto central
    fig.add_annotation(
        text=f"<b>{len(tickers)}</b><br><span style='font-size:9px'>activos</span>",
        x=0.5, y=0.5,
        font=dict(size=16, color=COLORES["text_primary"]),
        showarrow=False,
        xanchor="center",
        yanchor="middle",
    )
    ...
    # Eliminar ejes (no aplican para pie)
...
def grafico_correlacion(resultado: dict) -> go.Figure:
    """Heatmap de correlación con colores del tema."""
    retornos_ind = resultado["retornos_individuales"]

    if len(retornos_ind) < 2:
        fig = go.Figure()
        fig.add_annotation(
            text="Se necesitan ≥ 2 activos para calcular correlación",
    ...
    fig = go.Figure(go.Heatmap(
        ...
        colorbar=dict(
            thickness=10,
            tickfont=dict(size=9, color=COLORES["text_muted"]),
            outlinecolor=COLORES["border"],
            outlinewidth=1,
            tickformat=".1f",
        ),
    ))
